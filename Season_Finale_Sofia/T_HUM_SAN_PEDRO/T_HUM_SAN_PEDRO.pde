...
float tmaxsp, tminsp, hmaxsp, hminsp;
...
void setup() {
  ...
  sp = loadStrings("sanpedro.csv");
  diasp = new String[sp.length][23];
  ...
  for (int x = 0; x < cols; x++) {
    for (int y = 0; y < filas; y++) {
      ...
      if ((nums >= 0) && (nums < 365)) {
        ...
        //VARIABLES
        hmaxsp = float(diasp[nums][7]);      
        hminsp = float(diasp[nums][9]);
        ...
        //FUNCION
       fn (x1, y1, hmaxsp, hminsp, tmaxsp, tminsp);
      }
    }
  }
      endRecord();

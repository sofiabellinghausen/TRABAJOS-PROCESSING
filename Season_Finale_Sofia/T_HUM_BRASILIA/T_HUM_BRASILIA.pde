...
float tmaxbra, tminbra, hmaxbra, hminbra;
...
void setup() {
  ...
  bra = loadStrings("brasilia.csv");
  diabra = new String[bra.length][23];
  for (int i = 0; i < bra.length; i ++) {
    diabra[i] = bra[i].split(",");
  }
...
void draw() {
        beginRecord(PDF, "TempBrasilia.pdf");
  ...
  for (int x = 0; x < cols; x++) {
    for (int y = 0; y < filas; y++) {
      ...
      if ((nums >= 0) && (nums < 365)) {
        ...
        //VARIABLES
        hmaxbra = float(diabra[nums][7]);      
        hminbra = float(diabra[nums][9]);

        tmaxbra = float(diabra[nums][1]);        
        tminbra = float(diabra[nums][3]);


        //FUNCION
       fn (x1, y1, hmaxbra, hminbra, tmaxbra, tminbra);
      }
    }
  }
        endRecord();

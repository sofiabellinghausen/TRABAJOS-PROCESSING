  ...
  for (int x = 0; x < cols; x++) {
    for (int y = 0; y < filas; y++) {
      ...
      if ((nums >= 0) && (nums < 365)) {
        ...
        //variables
        vismv = float(diamv[nums][15]);       
        velmaxmv = float(diamv[nums][16]);
        velminmv = float(diamv[nums][17]);


        //FUNCION
        fn (x1, y1, vismv, velmaxmv, velminmv);
      }
    }
  }
          endRecord();

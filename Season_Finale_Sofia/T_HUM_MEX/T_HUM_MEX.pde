...
float tmaxmex, tminmex, hmaxmex, hminmex;
...
void setup() {
  ...
  mex = loadStrings("mex.csv");
  diamex = new String[mex.length][23];
  for (int i = 0; i < mex.length; i ++) {
    diamex[i] = mex[i].split(",");
  }
  ...
  for (int x = 0; x < cols; x++) {
    for (int y = 0; y < filas; y++) {
      ...
      if ((nums >= 0) && (nums < 365)) {
        ...
        //VARIABLES
        hmaxmex = float(diamex[nums][7]);      
        hminmex = float(diamex[nums][9]);

        tmaxmex = float(diamex[nums][1]);        
        tminmex = float(diamex[nums][3]);


        //FUNCION
       fn (x1, y1, hmaxmex, hminmex, tmaxmex, tminmex);
      }
    }
  }
    endRecord();

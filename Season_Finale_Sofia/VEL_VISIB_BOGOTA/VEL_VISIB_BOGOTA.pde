...
float visbog, velmaxbog, velminbog;
  ...
  for (int x = 0; x < cols; x++) {
    for (int y = 0; y < filas; y++) {
      ...
      if ((nums >= 0) && (nums < 365)) {
        ...
        //variables
        visbog = float(diabog[nums][15]);       
        velmaxbog = float(diabog[nums][16]);
        velminbog = float(diabog[nums][17]);


        //FUNCION
        fn (x1, y1, visbog, velmaxbog, velminbog);
      }
    }
  }
          endRecord();

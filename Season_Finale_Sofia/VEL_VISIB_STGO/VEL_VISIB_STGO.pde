...
float visstgo, velmaxstgo, velminstgo;
  ...
  for (int x = 0; x < cols; x++) {
    for (int y = 0; y < filas; y++) {
      ...
      if ((nums >= 0) && (nums < 365)) {
        ...
        visstgo = float(diastgo[nums][15]);       
        velmaxstgo = float(diastgo[nums][16]);
        velminstgo = float(diastgo[nums][17]);


        //FUNCION
        fn (x1, y1, visstgo, velmaxstgo, velminstgo);
      }
    }
  }
          endRecord();

...
float visasun, velmaxasun, velminasun;
...
void draw() {
        beginRecord(PDF, "VelAsuncion.pdf");
  ...
  for (int x = 0; x < cols; x++) {
    for (int y = 0; y < filas; y++) {
      ...
      if ((nums >= 0) && (nums < 365)) {
        ...
        //variables
        visasun = float(diaasun[nums][15]);       
        velmaxasun = float(diaasun[nums][16]);
        velminasun = float(diaasun[nums][17]);


        //FUNCION
        fn (x1, y1, visasun, velmaxasun, velminasun);
      }
    }
  }
        endRecord();

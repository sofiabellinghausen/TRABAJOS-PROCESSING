...
float visbra, velmaxbra, velminbra;
  ...
  for (int x = 0; x < cols; x++) {
    for (int y = 0; y < filas; y++) {
      ...
      if ((nums >= 0) && (nums < 365)) {
        ...
        visbra = float(diabra[nums][15]);       
        velmaxbra = float(diabra[nums][16]);
        velminbra = float(diabra[nums][17]);


        //FUNCION
        fn (x1, y1, visbra, velmaxbra, velminbra);
      }
    }
  }
          endRecord();

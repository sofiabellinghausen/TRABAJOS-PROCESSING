...
float tmaxarg, tminarg, hmaxarg, hminarg;
  ...
  for (int x = 0; x < cols; x++) {
    for (int y = 0; y < filas; y++) {
      ...
      if ((nums >= 0) && (nums < 365)) {
        ...
        //VARIABLES
        hmaxarg = float(diaarg[nums][7]);      
        hminarg = float(diaarg[nums][9]);

        tmaxarg = float(diaarg[nums][1]);        
        tminarg = float(diaarg[nums][3]);


        //FUNCION
       fn (x1, y1, hmaxarg, hminarg, tmaxarg, tminarg);
      }
    }
  }
    endRecord();

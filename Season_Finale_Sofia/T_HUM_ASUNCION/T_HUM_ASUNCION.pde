...
float tmaxasun, tminasun, hmaxasun, hminasun;
...
void setup() {
  ...
  asun = loadStrings("asuncion.csv");
  diaasun = new String[asun.length][23];
  for (int i = 0; i < asun.length; i ++) {
    diaasun[i] = asun[i].split(",");
  }
...
void draw() {
      beginRecord(PDF, "TempaAsuncion.pdf");
  ...
  for (int x = 0; x < cols; x++) {
    for (int y = 0; y < filas; y++) {
      ...
      if ((nums >= 0) && (nums < 365)) {
        ...
        //VARIABLES
        hmaxasun = float(diaasun[nums][7]);      
        hminasun = float(diaasun[nums][9]);

        tmaxasun = float(diaasun[nums][1]);        
        tminasun = float(diaasun[nums][3]);


        //FUNCION
       fn (x1, y1, hmaxasun, hminasun, tmaxasun, tminasun);
      }
    }
  }
      endRecord();

  ...
  for (int x = 0; x < cols; x++) {
    for (int y = 0; y < filas; y++) {
      ...
      if ((nums >= 0) && (nums < 365)) {
        ...
        //variables
        vislp = float(dialp[nums][15]);       
        velmaxlp = float(dialp[nums][16]);
        velminlp = float(dialp[nums][17]);


        //FUNCION
        fn (x1, y1, vislp, velmaxlp, velminlp);
      }
    }
  }
          endRecord();
